...
seed:
	cd apps/eis-ai && .venv/bin/python -m app.db.seed --reset

api:
	cd apps/eis-ai && .venv/bin/uvicorn app.main:app --reload --port 8000

web:
	cd apps/portal-web && npm run dev

test:
	cd apps/eis-ai && .venv/bin/python -m pytest -q

check: test
	cd apps/portal-web && npx tsc --noEmit && npm run build

demo:
	cd apps/eis-ai && .venv/bin/python -m scripts.demo
